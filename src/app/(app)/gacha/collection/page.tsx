...
import { IconBook, IconCatFace, IconCan } from '@/components/PixelIcons'
...
export default function CollectionPage() {
  ...
  const { ownsItem, getQuantity, useItem, collectionPct, ownedCount, totalItems } = useInventory()
  ...
  return (
    <div className="page-scroll">
      ...
      <div className="flex items-center gap-2 mb-1">
        ...
        <span className="pixel-chip flex items-center gap-1.5" style={{ background: 'linear-gradient(135deg, #7C3AED, #A78BFA)' }}>
          <IconBook size={11} /> COLLECTION
        </span>
      </div>
      ...
      <div className="flex gap-1 mb-4 overflow-x-auto pb-1" style={{ scrollbarWidth: 'none' }}>
        {CATEGORIES.map(cat => (
          <button key={cat} onClick={() => { playSound('ui_tap'); setTab(cat); setSelected(null) }}
            ...
            {(() => { const Icon = TAB_ICON[cat]; return <Icon size={10} /> })()}
