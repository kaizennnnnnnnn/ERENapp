...
// ─── The dock itself ─────────────────────────────────────────────────────────
// The four places, in a row on the living-room floor, just above the bottom
// nav: positioned against home's room stage, which ends at the nav's top edge
// (HomeRoomFrame), so no safe-area inset is needed here. Each flies in on its
// own cloud: rainbow for the lucky gacha, pink for the bakery, soot for the
// kiosk, mint for the parlour.

const DOCK_BOTTOM = 10

/**
 * How far above the stage floor the dock and the trinkets poking out of its
 * top (the shawarma's steam reaches ~19px up) end. Anything that has to stay
 * readable over the floor — the swipe dots, home's toast — sits above this.
 */
export const DOCK_CLEARANCE = DOCK_BOTTOM + (dockFrame.height as number) + 20

const PLACES: ReadonlyArray<{ href: string; theme: DockTheme; label: string; cloud: CloudTheme; delay: string }> = [
  { href: '/gacha', theme: 'gacha', label: 'GACHA', cloud: 'rainbow', delay: '0.18s' },
  { href: '/bakery', theme: 'cake', label: 'CAKE', cloud: 'pink', delay: '0.26s' },
  { href: '/shawarma', theme: 'shawarma', label: 'SHAWARMA', cloud: 'smoke', delay: '0.34s' },
  { href: '/jelly', theme: 'jelly', label: 'JELLY', cloud: 'mint', delay: '0.42s' },
]

export default function HomeDock() {
  return (
    <div className="absolute left-0 right-0 z-20 flex gap-2 px-2" style={{ bottom: DOCK_BOTTOM }}>
      {PLACES.map(p => (
        <Link
          key={p.href}
          href={p.href}
          onClick={e => {
            // CloudTransition owns the push: the clouds converge, the route
            // swaps underneath, then they part on the place.
            e.preventDefault()
            playSound('ui_tap')
            requestCloudNav(p.href, p.cloud)
          }}
          className="home-dock-btn home-dock-pop"
          style={{ ...dockFrame, animationDelay: p.delay }}
        >
          <DockContent theme={p.theme} label={p.label} />
        </Link>
      ))}
    </div>
  )
