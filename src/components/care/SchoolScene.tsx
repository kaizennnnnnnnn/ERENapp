...
// Desk hardware. PLANK_W mirrors scripts/make_desk_wood.py — the nails have to
// land inside the boards the texture actually draws, so if that constant moves,
// this one moves with it. Enough columns to cover a tablet; overflow clips.
const PLANK_W = 84
// Hand-driven, not machine-placed: each nail is nudged off the row by a couple
// of px. A ruler-straight line of them reads as a UI element, not hardware.
const NAIL_JITTER = [0, 2, -1, 3, -2, 1, 2, -3, 1, -1, 3, 0, -2, 2]
const DESK_NAILS = Array.from({ length: 7 }, (_, k) => k).flatMap(k => [
  { left: k * PLANK_W + 21 + NAIL_JITTER[k], top: 96 + NAIL_JITTER[k + 2], bottom: undefined as number | undefined },
  { left: k * PLANK_W + 61 - NAIL_JITTER[k + 5], top: undefined as number | undefined, bottom: 20 + NAIL_JITTER[k + 7] },
])
...
function CourseMap({ progress, strugglingCount, onLessonTap, onPracticeTap, onClose }: {
  ...
  return (
    <div className="fixed inset-0 z-40 flex flex-col overflow-hidden" style={{
      ...
      // Generated by scripts/make_desk_wood.py rather than stacked CSS
      // gradients, which can only draw evenly spaced parallel stripes — real
      // grain wanders, crowds, and sweeps around knots. Tiles on both axes at
      // its native size, and the tile is a whole number of PLANK_W boards, so
      // the horizontal repeat lands exactly on a seam.
      backgroundImage: 'url(/desk_wood.webp?v=1)',
      backgroundRepeat: 'repeat',
      backgroundColor: '#7d5324',
      fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", system-ui, sans-serif',
    }}>
      {/* Nails holding the boards down, two per plank, staggered between the
          rows so they don't line up as a grid. Positioned off PLANK_W from the
          left edge, which is where the texture's seams fall. */}
      {DESK_NAILS.map((n, i) => (
        <span key={i} className="absolute pointer-events-none" style={{
          left: n.left, top: n.top, bottom: n.bottom,
          width: 9, height: 9, borderRadius: '50%',
          // Countersunk: a lit dome sitting in a dark dent. The inset shadow
          // pair does the sinking, the outer one the drop onto the board.
          background: 'radial-gradient(circle at 34% 28%, #e6eaf0 0%, #aab0ba 40%, #6d737d 72%, #3a3f47 100%)',
          boxShadow: [
            '0 1px 2px rgba(20,10,2,0.60)',
            '0 0 0 1px rgba(38,20,6,0.45)',
      ...
      {/* Warm light pooling from the top-right (keeps the sunlit-room feel the
          cork had) and a vignette to sink the corners. The dot grain that used
          to live here is gone — the texture carries its own now. */}
      <div className="absolute inset-0 pointer-events-none" style={{
        backgroundImage: `
          radial-gradient(90% 55% at 84% 4%, rgba(255,230,175,0.26), transparent 60%),
          radial-gradient(ellipse 120% 80% at 50% 50%, transparent 42%, rgba(30,16,4,0.46) 100%)`,
      }} />
