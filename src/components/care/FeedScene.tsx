...
export default function FeedScene({ onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-40 overflow-hidden select-none">
      ...
      {/* ══ BOTTOM PANEL ══ */}
      <div className="absolute bottom-0 left-0 right-0 z-30 overflow-hidden" style={{ height: '46%', background: 'linear-gradient(180deg, #FFFBF0 0%, #FFF8E8 100%)', borderTop: '3px solid #F5C842', boxShadow: '0 -3px 0 #E8A020' }}>
        {/* Tabs */}
        <div className="flex gap-1 mx-3 mt-3 mb-1">
          {[
            { key: 'shop' as const, icon: <ShoppingCart size={12} />, label: 'Shop' },
            { key: 'fridge' as const, icon: <Package size={12} />, label: `Fridge${fridgeItems.length > 0 ? ` (${fridgeItems.reduce((s, i) => s + (inventory[i.id] ?? 0), 0)})` : ''}` },
          ].map(t => (
            <button key={t.key} onClick={() => setTab(t.key)}
              className="flex-1 flex items-center justify-center gap-1.5 py-2 transition-all active:translate-y-[1px]"
              style={tab === t.key
                ? { background: 'linear-gradient(135deg, #F5C842, #E8A020)', borderRadius: 3, border: '2px solid #C88018', boxShadow: '0 2px 0 #A06010', color: 'white', fontFamily: '"Press Start 2P"', fontSize: 7 }
                : { background: 'transparent', color: '#B09040', fontFamily: '"Press Start 2P"', fontSize: 7 }
              }>
              {t.icon} {t.label}
            </button>
          ))}
