...
// ═══════════════════════════════════════════════════════════════════════════════
// MEADOW ICONS — the flat set of the Meadow redesign (bottom nav, Us / Play / Me /
// Settings, onboarding).
//
// A different family from the outlined, shaded icons above, on purpose: Meadow
// draws with 1-3 flat colours and no outline, so a nav tab can be recoloured
// from grey to leaf by swapping ONE colour. The shapes are copied cell for cell
// from the approved demo boards (scratchpad canvas/project/meadow), so what
// ships is what the owner signed off. Keep using the icons above for the rooms
// and the old HUD; reach for these on Meadow surfaces.
//
// Palette letters: 'X' is the icon's main ink and is what `color` replaces;
// lower-case letters are accents that keep their colour. `mono` paints every
// cell in `color` (the needs card's grey heart, the white heart on the green
// "Send some love" button). Most grids are 12x12; check8 is 8x8 (the check in
// the selection disc) and male/female are 16x16.
// ═══════════════════════════════════════════════════════════════════════════════

interface MeadowIconSpec {
...
const MEADOW_ICONS = {
  ...
  coin: {
    ...
    palette: { X: '#F2B33D', a: '#C98C1E', b: '#FFE3A0' },
  },
  star: {
    grid: [
      '.....XX.....',
  ...
  shine: {
    ...
    palette: { X: '#F2B33D', a: '#C98C1E', b: '#FFF3C4' },
  },
  heart: {
    grid: [
      '............',
  ...
  crown: {
    ...
    palette: { X: '#F2B33D', a: '#C98C1E', b: '#E57C9F' },
  },
  bolt: {
    grid: [
      '.......XXX..',
  ...
  dice: {
    ...
    palette: { X: '#FFFFFF', a: '#2F2B28', b: '#DAD5CC' },
  },
  brush: {
    grid: [
      '..........aa',
    ...
    palette: { X: '#3F8452', a: '#9C5B3A', b: '#8A9096' },
  },
  pencil: {
    grid: [
      '.........XX.',
  ...
  calendarWeek: {
    ...
    palette: { X: '#6E6A66', a: '#F08A3C', b: '#FFFFFF' },
  },
  trash: {
    grid: [
      '....XXXX....',
  ...
  clock: {
    ...
    palette: { X: '#3F8452', a: '#FFFFFF' },
  },
  // Love tray, notes, moods
  lips: {
    grid: [
      '............',
      '............',
    ...
    palette: { X: '#E57C9F', a: '#A2405F', b: '#F6BCCD' },
  },
  loveLetter: {
    grid: [
      '............',
  ...
  storm: {
    ...
    palette: { X: '#8E8791', a: '#F08A3C' },
  },
  // Meadow stage and the cat builder
  daisy: {
    grid: [
      '....XXX.....',
      '...XXaXX....',
    ...
    palette: { X: '#E57C9F', a: '#F2B33D', b: '#3F8452' },
  },
  tuft: {
    grid: [
      '............',
  ...
  // Arcade games (one per playable game)
  memoryMatch: {
    grid: [
      '............',
      'XXXXXXX.....',
    ...
    palette: { X: '#E57C9F', a: '#F6BCCD', b: '#6E6A66', c: '#FFFFFF' },
  },
  treatTumble: {
    grid: [
      '............',
    ...
    palette: { X: '#F2A93B', a: '#FFE3A0', b: '#2F2B28', c: '#C98C1E' },
  },
  flappyEren: {
    grid: [
      '..........X.',
    ...
    palette: { X: '#5B9BD5', a: '#8A9096', b: '#9CC4EA', c: '#FFFFFF' },
  },
  ticTacToe: {
    grid: [
      '............',
    ...
    palette: { X: '#A7A29C', a: '#E57C9F', b: '#3F8452' },
  },
  erenStack: {
    grid: [
      '............',
    ...
    palette: { X: '#3F8452', a: '#8DBF8E', b: '#A7A29C' },
  },
  yarnPop: {
    grid: [
      '............',
  ...
  erenSays: {
    ...
    palette: { X: '#3F8452', a: '#E57C9F', b: '#F2B33D', c: '#5B9BD5' },
  },
  laneRunner: {
    grid: [
      'a....XX....a',
  ...
  pawDoku: {
    ...
    palette: { X: '#6E6A66', a: '#8A6FB0', b: '#A7A29C' },
  },
  yarnSort: {
    grid: [
      '............',
    ...
    palette: { X: '#A7A29C', a: '#F2B33D', b: '#E57C9F', c: '#3F8452' },
  },
  purrBeat: {
    grid: [
      '....XXXXXXXX',
...
} satisfies Record<string, MeadowIconSpec>

export type MeadowIconName = keyof typeof MEADOW_ICONS

/** Every Meadow icon name, in the order above — handy for a picker or a test. */
export const MEADOW_ICON_NAMES = Object.keys(MEADOW_ICONS) as MeadowIconName[]

interface PixelRun { x: number; y: number; w: number; ch: string }

// One <rect> per horizontal run of same-letter cells instead of one per cell:
// a 24px icon then costs ~15 nodes rather than ~70, and a run can't show the
// hairline seams that abutting 1x1 rects sometimes do at fractional scales.
// Grids never change at runtime, so each is walked once and cached.
const runCache = new Map<MeadowIconName, PixelRun[]>()

function runsFor(name: MeadowIconName): PixelRun[] {
  const hit = runCache.get(name)
  if (hit) return hit
  const runs: PixelRun[] = []
  MEADOW_ICONS[name].grid.forEach((row, y) => {
    let x = 0
    while (x < row.length) {
      const ch = row[x]
      if (ch === '.') { x++; continue }
...
export interface MeadowIconProps {
  name: MeadowIconName
  /** Rendered width and height in px. Boards use 24 (default), 20 in dense rows, 36 for tiles. */
  size?: number
  /** Replaces the main ink ('X' cells). Accents keep their own colours unless `mono`. */
  color?: string
  /** Paint every cell in `color`. */
  mono?: boolean
...
export const MeadowIcon = memo(function MeadowIcon({
  name, size = 24, color, mono = false, title, className, style,
}: MeadowIconProps) {
  const spec = MEADOW_ICONS[name]
  const n = spec.grid.length
  const palette: Readonly<Record<string, string>> = spec.palette
  const fillFor = (ch: string) =>
    (mono || ch === 'X') && color ? color : palette[ch]
  return (
    <svg
      width={size}
      height={size}
    ...
    >
      {runsFor(name).map(r => (
        <rect key={`${r.x}-${r.y}`} x={r.x} y={r.y} width={r.w} height={1} fill={fillFor(r.ch)} />
      ))}
    </svg>
  )
})
