...
/** The glass rectangle is not all glass. The till's display, two sauce
 *  bottles, the salt and pepper and the napkin stack all stand on the serving
 *  ledge in FRONT of the window, and their tops break the sill line into it.
 *  Anything painted across the whole rectangle — rain, fog, the pane misting
 *  over — lands on them too, and a shaker under a sheet of fog reads as a
 *  shaker with a grey box on it.
 *
 *  So everything that belongs to the WINDOW wears this mask, cut from the art
 *  itself: opaque over the street, transparent over whatever is standing in
 *  front of it. Generated by scripts/build_glass_mask.py — re-run it if the
 *  window wall is ever repainted. */
export const GLASS_MASK_SRC = '/kiosk_glass_mask.webp'
export const GLASS_MASK = {
  maskImage: `url(${GLASS_MASK_SRC})`,
  WebkitMaskImage: `url(${GLASS_MASK_SRC})`,
  maskSize: '100% 100%',
