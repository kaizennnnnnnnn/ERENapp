 ...
 *   <id>.webp        cleaned + trimmed full sprite (gacha reveal / collection / closet thumb)
 *   <id>_notail.webp body with the tail erased   (BlinkingEren `src`)
 *   <id>_tail.webp   tail isolated, everything else transparent (BlinkingEren `tailSrc`)
 *
 * WEBP AT q92, NOT PNG, AND THE REASON IS 20x. These are ~1470x1912 RGBA
 * sprites of soft-shaded art; PNG is close to the worst container for that and
 * was costing 112.8 MB across 132 files. At full resolution — nothing is
 * downscaled, the art is not softer — WebP q92 is ~5% of that, 5.6 MB. Measured
 * on the four hardest cases (a dark skin, a metallic one, fine fur, the largest
 * file) and indistinguishable from the PNG at 4x magnification on the face.
 *
 * The extension change also sidesteps the service worker: sw.js serves images
 * stale-while-revalidate, so art replaced at the SAME path shows the old file
 * until a revalidation lands. A .webp path is a new URL with no stale entry.
 *
...
const SKINS = [
  ...
  // ── FoodSuits set — food-costume Eren skins (the 3rd gacha). Same front-
  //    facing cat, so they run through the identical pipeline. `set: 'food'`
  //    keeps them in their own banner pool (see lib/gacha.ts skinSet). Per-skin
  //    eye/tail overrides are added below only where auto-detection misses.
  { id: 'donut',           file: 'ErenDonut.png',           rarity: 'epic',      name: 'Donut Eren',            set: 'food' },
  { id: 'cherry',          file: 'ErenCherry.png',          rarity: 'rare',      name: 'Cherry Pie Eren',       set: 'food' },
  { id: 'tamale',          file: 'ErenTamale.png',          rarity: 'rare',      name: 'Tamale Eren',           set: 'food' },
  { id: 'cherry2',         file: 'ErenCherry2.png',         rarity: 'rare',      name: 'Cherry Cone Eren',      set: 'food' },
  { id: 'mochi',           file: 'ErenMochi.png',           rarity: 'epic',      name: 'Mochi Eren',            set: 'food' },
  ...
  { id: 'lemon2',          file: 'ErenLemon2.png',          rarity: 'rare',      name: 'Lemon Dress Eren',      set: 'food' },
  { id: 'applecandy2',     file: 'ErenAppleCandy2.png',     rarity: 'epic',      name: 'Candy Apple Suit Eren', set: 'food' },
]
  ...
  return new Promise((resolve) => {
    ...
    img.onload = () => {
      ...
      if (haveTail) {
        ...
        tail = tc.toDataURL('image/webp', 0.92)
      ...
      const notail = full.toDataURL('image/webp', 0.92)
      ...
      const fullUrl = fc2.toDataURL('image/webp', 0.92)
...
;(async () => {
  ...
  for (const s of todo) {
    ...
    const write = (suffix, url) => {
      if (!url) return
      fs.writeFileSync(path.join(OUT_DIR, `${s.id}${suffix}.webp`), Buffer.from(url.split(',')[1], 'base64'))
    }
    write('', r.full)
    write('_notail', r.notail)
    write('_tail', r.tail)
    manifest.push({
      id: s.id, name: s.name, rarity: s.rarity, set: s.set || 'animal',
  ...
  const tsRows = merged.map(m => {
    ...
    const tail = m.hasTail ? `'/skins/${m.id}_tail.webp'` : 'undefined'
    const to = m.hasTail && m.tailOrigin ? `'${m.tailOrigin}'` : 'undefined'
    return `  { id: '${m.id}', name: ${JSON.stringify(m.name)}, rarity: '${m.rarity}', set: '${m.set}', aspect: ${m.aspect}, ` +
      `src: '/skins/${m.id}_notail.webp', tailSrc: ${tail}, tailOrigin: ${to}, thumb: '/skins/${m.id}.webp', eyes: ${eyes} },`
  }).join('\n')
